...
from PyQt6.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QGridLayout, QTabWidget, QLineEdit, QFileDialog)
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # Row 3 - Editor Path Setting
        settings_layout.addWidget(QLabel("Editor Path:"), 2, 0)

        editor_layout = QHBoxLayout()
        self.editor_path_line = QLineEdit()
        self.editor_path_line.setText(self.config_manager.get_editor_path())
        self.editor_path_line.setPlaceholderText("/usr/bin/gedit or /home/user/code/code.appimage")
        self.editor_path_line.textChanged.connect(self.on_editor_path_changed)
        editor_layout.addWidget(self.editor_path_line)

        self.browse_editor_btn = QPushButton("Browse...")
        self.browse_editor_btn.clicked.connect(self.browse_editor_path)
        editor_layout.addWidget(self.browse_editor_btn)

        editor_widget = QWidget()
        editor_widget.setLayout(editor_layout)
        settings_layout.addWidget(editor_widget, 2, 1, 1, 2)
    ...
    def browse_editor_path(self):
        """Open file dialog to browse for editor executable."""
        file_path, _ = QFileDialog.getOpenFileName(
            self,
            "Select Text Editor",
            str(Path.home()),  # Start from home directory - much faster
            "All Files (*)"
        )

        if file_path:
            self.editor_path_line.setText(file_path)

    def on_editor_path_changed(self, text: str):
        """Handle editor path text change."""
        self.config_manager.set_editor_path(text.strip())
    ...
    def start_monitoring(self):
        ...
        self.monitor_thread = KeyboardMonitorThread(keyboard.path, self.script_manager, self.config_manager)
